...
class Parser:
    ...
    def parse_for_statement(self) -> List[str]:
        ...
        def check_original_assignment_program():
            #checks if the iteration variable is assigned a constant integer in the assignment statement
            if "int2vr" in original_assignment_program[0]:
                try:
                    int(original_assignment_program[0].split("(")[1].split(")")[0])
                    return True
                except:
                    return False
            return False

        def check_expr_program():
            #checks if condition expression is a less-than expression and if iteration variable on the lhs and integer constant on the rhs of expr.
            if "int2vr" in expr_program[0]:
                try:
                    int(expr_program[0].split("(")[1].split(")")[0])
                    vrQ = expr_program[0].split()[0]
                    if ("lti(" + iter_var + "," + vrQ + ");") in expr_program[1]:
                        return True
                except:
                    return False
            return False
        
        def check_loop_end_assignment_program():
            #checks if updates the iteration variable by adding value 1
            if "int2vr(1)" in loop_end_assignment_program[0]:
                vrQ = loop_end_assignment_program[0].split()[0]
                if ("addi(" + iter_var + "," + vrQ + ");") in loop_end_assignment_program[1]:
                        return True
            return False
        
        will_unroll = False
        if self.uf is not None and self.uf > 0 and len(original_assignment_program) > 1:
            iter_var = original_assignment_program[1].split()[0]

            num_loop_iterations = int(expr_program[0].split("(")[1].split(")")[0])
            #checks for any control flow

            no_branches = not any("branch" in line for line in loop_program) or any("beq" in line for line in loop_program) or any("branch" in line for line in loop_program) 
            #checks for any control flow

            no_itervar_bodyupdates = not any("addi(" + iter_var in line for line in loop_program)
            #checks if the loop body updates the iteration variable

            even_divide = num_loop_iterations/self.uf % 2 == 0 
            #checks if the unroll factor evenly divides the number of loop iterations

            if (check_original_assignment_program() and
                check_expr_program() and 
                check_loop_end_assignment_program() and
                no_branches and
                no_itervar_bodyupdates and
                even_divide):
                will_unroll = True
        ...
        if will_unroll:
            unrolled = (loop_program + loop_end_assignment_program) * self.uf
            return original_assignment_program + ["%s:" % (loop_start_label)] + expr_program + compare_ins + unrolled + branch_ins + ["%s:" % (end_label)]
